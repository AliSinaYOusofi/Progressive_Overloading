...
// Upsert profile (try update first, insert if not found)
export const upsertProfile = async (userId, profileData) => {
  try {
    ...
    // Ensure email is included (NOT NULL constraint) by falling back to auth session
    let insertData = { id: userId, ...profileData };
    if (!insertData.email) {
    ...
    if (insertError) {
      // Duplicate email — another profile already uses this email
      if (insertError.code === '23505') {
        console.warn('Profile insert skipped: email already in use by another profile');
        return null;
      }
      throw insertError;
    }
...
export const getExerciseSetsByUser = async (userId, limit = 500, days = null) => {
  try {
    let query = supabase
      .from('exercise_sets')
...
// Get exercise sets within a date range (inclusive)
export const getExerciseSetsByDateRange = async (userId, startDate, endDate) => {
  try {
    ...
    console.error('Error fetching exercise sets by date range:', error);
    throw error;
  }
};
...
export const getExerciseProgressRows = async (userId, limit = 1000, days = null) => {
  // Returns rows suitable for Home progress: exerciseName, first1RM, best1RM, delta
  try {
    const sets = await getExerciseSetsByUser(userId, limit, days);
...
export const getStreakAnalytics = async (userId, days = 365) => {
  try {
    ...
    // Paginate to get all sets in the period (Supabase defaults to 1000 rows)
    let allData = [];
    let from = 0;
    const pageSize = 1000;
    let hasMore = true;
    ...
    // Helper: format date as local YYYY-MM-DD (matches dateUtils.toLocalDateStr)
    ...
    console.log('[StreakAnalytics] Query returned', allData.length, 'sets');

    // Get unique workout dates (local timezone)
    ...
    console.log('[StreakAnalytics] Unique workout dates:', Array.from(workoutDates));

    // Create activity map for last N days (local timezone)
    const activityMap = {};
    const today = new Date();
    ...
    if (!workoutDates.has(todayStr)) {
      ...
      checkDateStr = toLocalDateStr(yesterday);
    }
    ...
    for (const dateStr of sortedDates) {
      ...
      if (dateStr === checkDateStr) {
        ...
        const checkDate = new Date(checkDateStr + 'T12:00:00'); // noon to avoid DST edge cases
        checkDate.setDate(checkDate.getDate() - 1);
        checkDateStr = toLocalDateStr(checkDate);
    ...
    console.log('[StreakAnalytics] Result:', { currentStreak, longestStreak, totalWorkoutDays, thisWeekWorkouts });
    return result;
  } catch (error) {
...
export const getUserStats = async (userId, days = null) => {
  try {
    const endDate = new Date();
    // Count distinct workout days (unique dates from performed_at)
    ...
    const { data: setsData, error: setsError } = await daysQuery;

    if (setsError) throw setsError;

    // Count unique dates (workout days)
    const uniqueDays = new Set(
      (setsData || []).map(s => new Date(s.performed_at).toISOString().split('T')[0])
    );
    const workoutDayCount = uniqueDays.size;
