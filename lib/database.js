...
export const getExerciseProgressRows = async (userId, limit = 1000) => {
  ...
  try {
    ...
    const grouped = sets.reduce((acc, s) => {
      const key = s.exercise_id || s.exercises?.id || s.exercises?.name || 'unknown';
      if (!acc[key]) acc[key] = [];
      acc[key].push(s);
      return acc;
    }, {});
    const rows = Object.values(grouped).map(list => {
      list.sort((a, b) => new Date(a.performed_at) - new Date(b.performed_at));
      const first = list[0];
      const best = list.reduce((m, x) => {
        const m1 = computeOneRM(m.weight, m.reps);
        const x1 = computeOneRM(x.weight, x.reps);
        return x1 > m1 ? x : m;
      }, list[0]);
      const first1RM = computeOneRM(first.weight, first.reps);
      const best1RM = computeOneRM(best.weight, best.reps);
      return {
        exerciseName: best.exercises?.name || 'Exercise',
        first1RM,
        best1RM,
        delta: best1RM - first1RM,
        bestAt: best.performed_at,
      };
    });
