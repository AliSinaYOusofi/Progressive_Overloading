  ...
  try {
    ...
    Object.values(progression).forEach(exercise => {
      if (exercise.sets.length < 2) {
        exercise.progression = 'insufficient_data';
        exercise.recommendation = 'Log more sets to track progression';
        return;
      }
      ...
      const firstWeek = exercise.sets.slice(0, Math.ceil(exercise.sets.length / 2));
      const lastWeek = exercise.sets.slice(-Math.ceil(exercise.sets.length / 2));
      
      const firstAvg = firstWeek.reduce((sum, set) => sum + set.oneRM, 0) / firstWeek.length;
      const lastAvg = lastWeek.reduce((sum, set) => sum + set.oneRM, 0) / lastWeek.length;
      
      exercise.weeklyGain = ((lastAvg - firstAvg) / firstAvg) * 100;
      
      if (exercise.weeklyGain > 5) {
        exercise.progression = 'excellent';
        exercise.recommendation = 'Great progress! Consider increasing weight or reps';
      } else if (exercise.weeklyGain > 2) {
        exercise.progression = 'good';
        exercise.recommendation = 'Good progress! Keep pushing';
      } else if (exercise.weeklyGain > -2) {
        exercise.progression = 'stable';
        exercise.recommendation = 'Maintaining strength. Consider increasing intensity';
      } else {
        exercise.progression = 'declining';
        exercise.recommendation = 'Focus on form and consider deloading';
      }
    });
...
export const getVolumeAnalysis = async (userId, days = 30) => {
  try {
    ...
    const avgDailyVolume = totalVolume / Math.max(volumeData.length, 1);
    ...
    return {
      ...
      trend: volumeData.length > 1 ? 
        (volumeData[volumeData.length - 1].totalVolume > volumeData[0].totalVolume ? 'increasing' : 'decreasing') : 'stable'
    };
  } catch (error) {
    console.error('Error fetching volume analysis:', error);
    return { totalVolume: 0, avgDailyVolume: 0, maxVolume: 0, dailyData: [], trend: 'stable' };
  }
};
