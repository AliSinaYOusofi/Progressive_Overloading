...
export const getMonthlyStats = async (userId, months = 6) => {
  try {
    ...
    // If months is very large (all time), find the first set date
    const isAllTime = months >= 100;
    
    if (isAllTime) {
      // Find the first set date for this user
    ...
    allMonths.forEach(m => monthlyDataMap.set(m.month, m));
    ...
    data?.forEach(set => {
      ...
      if (monthData) {
        ...
        monthData.totalVolume += (set.weight || 0) * (set.reps || 0) * (set.sets || 1);
        monthData.uniqueExercises.add(set.exercises?.name || 'Unknown');
      }
    });
